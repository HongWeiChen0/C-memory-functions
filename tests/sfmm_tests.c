...
Test(sfmm_student_suite, student_test_1_malloc_zero, .timeout = TEST_TIMEOUT) {
	void* x = sf_malloc(0);

	cr_assert_null(x, "x is not NULL!");
}

Test(sfmm_student_suite, student_test_2_realloc_too_large, .timeout = TEST_TIMEOUT) {
	sf_errno = 0;
    void *x = sf_malloc(8);
    x = sf_realloc(x, 999999);
...
Test(sfmm_student_suite, student_test_3_realloc_invalid_pointer, .timeout = TEST_TIMEOUT) {
	sf_errno = 0;
	void* x = sf_malloc(8);
	x++;
	x = sf_realloc(x, 64);
...
Test(sfmm_student_suite, student_test_4_memalign_size_zero, .timeout = TEST_TIMEOUT) {
    void *x = sf_memalign(0, 64);

	cr_assert_null(x, "x is not NULL!");
}

Test(sfmm_student_suite, student_test_5_realloc_not_power_of_two, .timeout = TEST_TIMEOUT) {
	sf_errno = 0;
    void* x = sf_memalign(80, 40);

	cr_assert_null(x, "x is not NULL!");
	cr_assert(sf_errno == EINVAL, "sf_errno is not ENOMEM!");
}